...
def validate_input(f, data, baseline, model_features, schema, entity_col, time_col):
    ...
    assert isinstance(data, (pd.DataFrame, np.ndarray)), (
        "Provided data must be an numpy array or pandas DataFrame"
    )
    assert baseline is None or isinstance(baseline, (pd.DataFrame, np.ndarray)), (
        "Provided baseline must be an numpy array or pandas DataFrame"
    )
    assert model_features is None or (
        isinstance(model_features, list) and isinstance(model_features[0], (int, str))
    ), (
        "Model features must be a list of features (str) or their corresponding indexes(ints)"
    )
    assert entity_col is None or isinstance(entity_col, (int, str)), (
        "Provided entity column must be a feature name (str) or the corresponding index (int)"
    )
    assert time_col is None or isinstance(time_col, (int, str)), (
        "Provided time column must be a feature name (str) or the corresponding index (int)"
    )
    ...
    if model_features is None:
        ...
        isinstance(model_features[0], str)
        or (entity_col is not None and isinstance(entity_col, str))
        or (time_col is not None and isinstance(time_col, str))
    ):
        # we have strings to obtain indexes, therefore we need the schema
        if isinstance(data, pd.DataFrame):
            schema = list(data.columns)
        assert schema is not None, (
            "When model features, entity column or time column are strings, data schema must be provided"
        )
        ...
        assert data.shape[-1] >= len(model_features), (
            "Provided model features do not match data"
        )
        assert data.shape[-1] == len(schema), "Provided schema does not match data"
        assert set(model_features).issubset(set(schema)), (
            "Provided model features must be in the provided schema"
        )

        if time_col is not None and isinstance(time_col, str):
            assert time_col in schema, (
                "Provided time feature must be in the provided schema"
            )

        if entity_col is not None and isinstance(entity_col, str):
            assert entity_col in schema, (
                "Provided entity feature must be in the provided schema"
            )
    else:
        # we are dealing with indexes
        # these columns must not be model features
        assert entity_col is None or entity_col not in model_features, (
            "Provided entity col index must not be on model feature indexes"
        )
        assert time_col is None or time_col not in model_features, (
            "Provided time col index must not be on model feature indexes"
        )
        assert data.shape[-1] >= len(model_features), (
            "Higher number of model features provided than present in data"
        )
...
def get_tolerances_to_test(
    ...
    if pruning_data is None:
        if explanation_dict.get("tol", False):
            ...
            print(
                f"No pruning data provided. TimeSHAP will calculate pruning on-the fly using provided tolerances: {list(tolerances_to_calc)} "
            )
        else:
            print(
        ...
        if input_tols is not None:
            assert np.array([x in tolerances_to_calc for x in input_tols]).all(), (
                "Inputed tolerances are not present on the provided pruning data"
            )
        ...
        if len(data.shape) == 3:
            for sequence_idx in range(data.shape[0]):
                ...
                if entity_col_index is not None:
                    ...
                    assert np.all(entity == entity[0]), (
                        "All entities of a sequence must be the same"
                    )
...
def calc_avg_sequence(
    ...
    if isinstance(data, pd.DataFrame):
        assert entity_col is not None, (
            "To calculate average sequence from DataFrame, entity_col is required"
        )
        sequences = data.groupby(entity_col)
        seq_lens = sequences.size().values
        assert np.array([x == seq_lens[0] for x in seq_lens]).all(), (
    ...
    if (
        len(numerical_feats) > 0
        and isinstance(numerical_feats[0], str)
        or len(categorical_feats) > 0
        and isinstance(categorical_feats[0], str)
    ):
        # given features are not indexes
        assert model_features is not None and len(model_features), (
            "When using feature names to identify them, specify the model features. Alternatively you can pass the indexes of the features directly"
        )
    ...
    if len(categorical_indexes) > 0:
        categorical = stats.mode(data[:, :, categorical_indexes], axis=0)[0].astype(
            np.int64
        )
        numerical = np.concatenate((categorical, numerical), axis=1)
    return numerical
...
def calc_avg_event(
    ...
    if (
        ...
        numerical_indexes = [model_features.index(str(x)) for x in numerical_feats]
        categorical_indexes = [model_features.index(str(x)) for x in categorical_feats]
    else:
        numerical_indexes = numerical_feats
        categorical_indexes = categorical_feats
    ...
    numerical = np.median(data[:, np.array(numerical_indexes)], axis=0)
    categorical = None
    if len(categorical_indexes) > 0:
        categorical = stats.mode(data[:, np.array(categorical_indexes)], axis=0)[
            0
        ].flatten()
    else:
    ...
    all_values = np.concatenate([categorical, numerical])

    return pd.DataFrame([all_values], columns=model_features)
...
def get_avg_score_with_avg_event(
    model, med: Union[pd.DataFrame, np.ndarray], top: int = 1000
) -> Dict[int, float]:
