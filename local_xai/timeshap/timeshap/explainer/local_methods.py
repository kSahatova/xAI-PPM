...
def validate_local_input(
    ...
    entity_col: Union[str, int],
    time_col: Union[str, int],
    entity_uuid: str,
):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if model_features:
            assert set(model_features).issubset(data_cols), (
                "When providing model features, these should be on the given DataFrame"
            )
        else:
            print("Assuming all features are model features")
            assert entity_col is None, (
                "Entity col provided but no model features provided"
            )
            assert time_col is None, "Time col provided but no model features provided"
        if entity_col is not None:
            assert entity_col in data_cols, (
                "When providing entity feature, these should be on the given DataFrame"
            )
            assert len(np.unique(data[entity_col].values)) == 1, (
                "For local report, provided data must contain one instance only"
            )
        if time_col is not None:
            assert time_col in data_cols, (
                "When providing time feature, these should be on the given DataFrame"
            )
    else:
        assert len(data.shape) == 3, (
            "Provided data must be an numpy array with 3 dimensions"
        )
    ...
    assert baseline is None or isinstance(baseline, (pd.DataFrame, np.ndarray)), (
        "Baseline must be a pd.DataFrame or np.ndarrays"
    )

    assert pruning_dict is None or pruning_dict.get("tol") is not None, (
        "Prunning dict must have tolerance attribute"
    )
    if pruning_dict is not None:
        assert isinstance(pruning_dict.get("tol"), (int, float)), (
            "Provided tolerance must be a int or float"
        )
        if isinstance(pruning_dict.get("tol"), int):
            assert pruning_dict.get("tol") == 0, (
                "Provided tolerance must be a float or 0"
            )
    ...
    check_dict(
        ...
        "plot_features",
        dict,
        "Provided plot_features must be a dict, mapping model features, to plot features",
    )
    ...
    if cell_dict is not None:
        ...
        if "threshold" in cell_dict or "top_x" in cell_dict:
            ...
            assert "feat_threshold" not in cell_dict, (
                f"Provided both feat_threshold and {provided}. Please only provide one"
            )
            assert "top_x_feats" not in cell_dict, (
                f"Provided both top_x_feats and {provided}. Please only provide one"
            )
            assert "event_threshold" not in cell_dict, (
                f"Provided both event_threshold and {provided}. Please only provide one"
            )
            assert "top_x_events" not in cell_dict, (
                f"Provided both top_x_events and {provided}. Please only provide one"
            )
        else:
...
def calc_local_report(
    ...
    if pruning_dict is None:
        ...
        coal_plot_data, coal_prun_idx = local_pruning(
            f, data, pruning_dict, baseline, entity_uuid, entity_col, verbose
        )
        pruning_idx = data.shape[1] + coal_prun_idx

    event_data = local_event(
        f, data, event_dict, entity_uuid, entity_col, baseline, pruning_idx
    )

    feature_data = local_feat(
        f, data, feature_dict, entity_uuid, entity_col, baseline, pruning_idx
    )
...
def local_report(
    ...
    #                          )
    return pruning_data, event_data, feature_data, cell_level
