...
from timeshap.plot.cell_level import plot_cell_level
from timeshap.explainer import prune_given_data


def plot_local_report(
    pruning_dict: dict,
    event_dict: dict,
    feature_dict: dict,
    cell_dict: dict,
    ...
    if pruning_dict is None:
        if pruning_dict is not None:
            assert pruning_dict.get("path", False), (
                "No data or path to data provided to calculate pruning statistics"
            )
    if event_data is None:
        assert event_dict.get("path", False), (
            "No data or path to data provided to plot event explanations"
        )
    if feat_data is None:
        assert feature_dict.get("path", False), (
            "No data or path to data provided to plot feature explanations"
        )
    if cell_data is None and cell_dict is not None:
        assert cell_dict.get("path", False), (
            "No data or path to data provided to plot feature explanations"
        )
    ...
    if coal_plot_data is not None:
        ...
        pruning_plot = plot_temp_coalition_pruning(
            coal_plot_data, coal_prun_idx, plot_lim
        )

    event_plot = plot_event_heatmap(event_data)

    feature_plot = plot_feat_barplot(
        feat_data, feature_dict.get("top_feats"), feature_dict.get("plot_features")
    )

    if cell_dict:
        feat_names = list(feat_data["Feature"].values)[:-1]  # exclude pruned events
        cell_plot = plot_cell_level(
            cell_data, feat_names, feature_dict.get("plot_features")
        )
        if coal_plot_data is not None:
            plot_report = (
                pruning_plot | event_plot | feature_plot | cell_plot
            ).resolve_scale(color="independent")
        else:
            plot_report = (event_plot | feature_plot | cell_plot).resolve_scale(
                color="independent"
            )

    else:
        if coal_plot_data is not None:
            plot_report = (pruning_plot | event_plot | feature_plot).resolve_scale(
                color="independent"
            )
        else:
