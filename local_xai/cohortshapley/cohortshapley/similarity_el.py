...
def  control_flow_log_distance_simplified(trace1, trace2, total_events_num=25, special_tokens_num=3):
    """
    ...
    event_to_symbol_mapping = {i: characters[i-special_tokens_num] for i in range(start_ind, end_ind)}
    trace1_cat = trace1[0].squeeze().numpy()
    trace2_cat = trace2[0].squeeze().numpy()
    activity_sequence1 = "".join([event_to_symbol_mapping[activity] for activity in trace1_cat])
    activity_sequence2 = "".join([event_to_symbol_mapping[activity] for activity in trace2_cat]) 
