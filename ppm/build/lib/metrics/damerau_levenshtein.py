...
def _handle_pred_length(predicted, pad_idx=1, eos_idx=2):
        ...
        pred_lengths = (
            torch.flip(pred_lengths, dims=[1]) # reverse each row in order to use cumsum
            .cumsum(dim=1)  # fill with ones all the columns after the first eos_idx
            .gt(0) # convert to boolean any value greater than 0
        ).flip(dims=[1]).sum(dim=1) # reverse again and sum the columns to get the length of the sequence
