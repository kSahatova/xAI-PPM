...
class GAT_AE(nn.Module):
    ...
    def forward(self, graphs , Xs, mask,batch_size):
        ...
        :return:
        '''
        attr_reconstruction_outputs = [] # probability distribution probability map
        s = []  # Decoding layer GRU initial hidden representation
        enc_output = None
        # Z=None
        ...
        for i, dim in enumerate(self.attribute_dims):
            if i == 0:
                ...
                dec_input = X[:,0] # target The first column is the starting character. teacher_forcing   [batch_size]

                for t in range(1,  self.max_seq_len):
                    dec_output, s0 = self.decoders[i](dec_input, s0, enc_output,mask)

                    # Store the output at each time step
                ...
                dec_input_attr = X_attr[:,0]  # target  teacher_forcing  [batch_size]

                for t in range(1,  self.max_seq_len):
                    dec_input_act = X_act[:,t]  # teacher_forcing activity [batch_size]

                    dec_output, s0 = self.decoders[i](dec_input_act, dec_input_attr, s0, enc_output,mask)  # s0 Hidden state
