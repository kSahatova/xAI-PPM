...
for dataset_name in datasets:
    ...
    if (
        not os.path.isfile(optimal_params_filename)
        or os.path.getsize(optimal_params_filename) <= 0
    ):
        continue
    ...
    if gap > 1:
        outfile = os.path.join(
            results_dir,
            "performance_results_%s_%s_%s_gap%s.csv"
            % (cls_method, dataset_name, method_name, gap),
        )
    else:
        outfile = os.path.join(
            results_dir,
            "performance_results_%s_%s_%s.csv"
    ...
    for ii in range(n_iter):
        ...
        dt_train_prefixes = dataset_manager.generate_prefix_data(
            train, min_prefix_length, max_prefix_length, gap
        )
        ...
        for bucket in set(bucket_assignments_test):
            ...
            if len(relevant_train_cases_bucket) == 0:
                preds = [dataset_manager.get_class_ratio(train)] * len(
                ...
                dt_train_bucket = dataset_manager.get_relevant_data_by_indexes(
                    dt_train_prefixes, relevant_train_cases_bucket
                )  # one row per event
                ...
                if len(set(train_y)) < 2:
                    ...
                    if cls_method == "rf":
                        ...
                        cls = LogisticRegression(
                            C=2 ** current_args["C"], random_state=random_state
                        )
                    ...
                    if cls_method == "svm" or cls_method == "logit":
                        ...
                        pipeline = Pipeline(
                            [("encoder", feature_combiner), ("cls", cls)]
                        )
                    ...
                    for _, group in test_all_grouped:
                        ...
                        current_online_event_times.append(
                            pipeline_pred_time / len(group)
                        )
                        preds.extend(pred)
    ...
    with open(outfile, "w") as fout:
        ...
        online_event_times_flat = [
            t
            for iter_online_event_times in online_event_times
            for t in iter_online_event_times
        ]
        fout.write(
            "%s;%s;%s;%s;%s;%s;%s\n"
            % (
                ...
                -1,
                "online_time_avg",
                np.mean(online_event_times_flat),
            )
        )
        fout.write(
            "%s;%s;%s;%s;%s;%s;%s\n"
            % (
                ...
                -1,
                "online_time_std",
                np.std(online_event_times_flat),
            )
        )
        fout.write(
            "%s;%s;%s;%s;%s;%s;%s\n"
        ...
        fout.write(
            ...
            % (
                ...
                -1,
                -1,
                "offline_time_total_std",
                np.std(offline_total_times),
            )
        )
